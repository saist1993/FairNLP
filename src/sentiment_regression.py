...
data_dir = Path('../data/affect_in_tweet/task1/EI-reg/')
emotions = ['anger', 'fear', 'joy', 'sadness']
splits = ['train', 'dev', 'test-gold']
prepend_text = Template('EI-reg-En-$emotion-$split.txt')


def read_data(data_dir, emotion, split):
    a = open(data_dir/Path(prepend_text.substitute(emotion=emotion, split=split)), 'r')
    lines = a.readlines()
...
if __name__ == '__main__':
    ...
    for emotion in emotions:
        train = train + read_data(data_dir, emotion, 'train')
    dev = []
    for emotion in emotions:
        dev = dev + read_data(data_dir, emotion, 'dev')
    test = []
    for emotion in emotions:
        test = test + read_data(data_dir, emotion, 'test-gold')
